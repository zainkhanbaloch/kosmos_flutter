...
class _MainCardState extends State<MainCard> {
  late Future<Map<String, dynamic>>
      data; // Use Future to represent the asynchronous loading
  ...
  void initState() {
    super.initState();
    data = fetchAPOD("2023-10-10"); // Initialize with a Future
  }

  Future<Map<String, dynamic>> fetchAPOD(String date) async {
    return nasaApi.fetchAPOD(date).catchError((error) {
      print(error);
      // Handle the error as needed.
  ...
  void navigateToDetailedContent(Map<String, dynamic> apodData) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => Details(
              ...
              FutureBuilder<Map<String, dynamic>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return CircularProgressIndicator(); // Show a loading indicator (spinner)
                      ...
                      children: [
                        Padding(
                          ...
                          child: ClipRRect(
                            borderRadius: BorderRadius.circular(20),
                            child: Image.network(
                              apodData?['url'] ?? '', // Display the image
                              fit: BoxFit.cover,
                        ...
                        ElevatedButton(
                          onPressed: () => navigateToDetailedContent(apodData!),
                          child: const Text("Read More"),
                        ),
                      ],
                    );
